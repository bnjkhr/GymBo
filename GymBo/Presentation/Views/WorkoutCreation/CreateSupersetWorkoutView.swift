...
//  V6 - Superset Training UI
//

import SwiftUI

/// View for creating a new superset workout template
///
/// **Features:**
/// - 3-step wizard flow
/// - Step 1: Basic settings (name, rest times)
/// - Step 2: Create superset groups
/// - Step 3: Preview & save
///
/// **Workflow:**
/// ```
/// Step 1 → Step 2 → Step 3 → Create Workout → Navigate to Detail
/// ```
struct CreateSupersetWorkoutView: View {
    ...
    @State private var defaultRestTime: TimeInterval = 90  // Between sets
    @State private var restAfterGroup: TimeInterval = 120  // After superset complete
    ...
    private let restTimeBetweenSetsOptions = [
    ...
    private let restAfterSupersetOptions = [
        (value: TimeInterval(60), label: "60 Sek"),
        (value: TimeInterval(90), label: "90 Sek"),
        (value: TimeInterval(120), label: "2 Min"),
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Superset Workout")
    ...
    private var step1BasicSettings: some View {
        VStack(alignment: .leading, spacing: 24) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                TextField("z.B. Upper Body Superset", text: $workoutName)
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                restTimeButtons(
                    options: restTimeBetweenSetsOptions,
                    selection: $defaultRestTime
                )
            }

            // Rest Time After Superset
            VStack(alignment: .leading, spacing: 12) {
                Text("PAUSENZEIT NACH SUPERSET")
                    .font(.subheadline)
                    .fontWeight(.semibold)
                    .foregroundColor(.secondary)
                    .textCase(.uppercase)
                ...
                restTimeButtons(
                    options: restAfterSupersetOptions,
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text("Superset = 2 Übungen")
                        .font(.subheadline)
                        .fontWeight(.semibold)

                    Text("Übungen werden abwechselnd ohne Pause ausgeführt")
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
            }
        ...
        VStack(alignment: .leading, spacing: 24) {
            ...
            Button {
                ...
                HStack {
                    ...
                    Text("Weiteres Superset hinzufügen")
                        .font(.body)
                        .fontWeight(.medium)
                }
                .foregroundColor(Color(hex: "#F77E2D"))
    ...
    private var step3Preview: some View {
        VStack(alignment: .leading, spacing: 24) {
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                // Rest Times
                VStack(alignment: .leading, spacing: 4) {
                    Text("Pausenzeit: \(Int(defaultRestTime))s zwischen Sets")
                        .font(.caption)
                        .foregroundColor(.secondary)
                    Text("Pausenzeit: \(Int(restAfterGroup))s nach Superset")
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
            }
            ...
            HStack {
                ...
                let totalSets = exerciseGroups.reduce(0) { sum, group in
                    sum + (group.exercises.first?.targetSets ?? 0) * 2  // 2 exercises per superset
                }

                Text("Gesamt: \(exerciseGroups.count) Supersets, \(totalSets) Sätze")
                    .font(.subheadline)
                    .fontWeight(.medium)
            }
            .padding()
    ...
    private func createWorkout() {
        ...
        Task {
            ...
            do {
                let useCase = container.makeCreateSupersetWorkoutUseCase()
...
// #Preview {
//     CreateSupersetWorkoutView(onWorkoutCreated: { _ in })
